...
/**
 * Performance test for the Gateway entity.
 */
class GatewayGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Gateway entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all gateways")
            .get("/api/gateways")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new gateway")
            .post("/api/gateways")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_gateway_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created gateway")
                .get("${new_gateway_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created gateway")
            .delete("${new_gateway_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
